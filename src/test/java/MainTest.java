...
import pl.observedpinproblem.Main;
...
public class MainTest {
    ...
    public void testPins() {
        for (String entered : expectations.keySet()) {
            test(entered, Arrays.asList(expectations.get(entered)), Main.getPINs(entered));
        }
    } // testPins

    public void test(String entered, List<String> expected, List<String> result) {
        result.sort(comp);
        expected.sort(comp);
        Assert.assertEquals(entered, expected, result);
    }
}
